...
require('em')('ds',function(em){
  ...
  ds.Iterator = as.Class.create('Iterator',{
    ...
    init: function(d){
      ...
      // this.events.set('error','fireRemove');
      // this.events.set('begin','fireRemove');
      // this.events.set('node');
      // this.events.set('end','fireRemove');
      // this.events.set('append');
      // this.events.set('prepend');
      // this.events.set('remove');
      // this.events.set('removeAll');
  ...
  ds.GraphTraversalRoot = as.Class.create('GraphTraversal',{
  ...
  ds.GraphTraversal.DepthFirst = ds.GraphTraversal.DF = ds.GraphTraversalRoot.extend('DepthFirstGraph',{
    amplify: function(ac){
  ...
  ds.GraphTraversal.BF = ds.GraphTraversal.BreadthFirst = ds.GraphTraversalRoot.extend('BreadthFirstGraph',{
  ...
  ds.GraphTraversal.DLDF = ds.GraphTraversal.DepthLimitedDF = ds.GraphTraversal.DepthFirst.extend('DepthLimited DepthFirst',{
    init: function(processor,depth){
      this.Super(processor);
      this.depth  = this._dp = depth;
    },
    ready: function(){
      return (this.super.ready() && this.hasDepth());
    },
    _process: function(arc,promise){
      if(this.hasDepth()){
        this._dp -= 1;
        return this.super._process(arc,promise);
      } 
      return promise.promise();
    },
    hasDepth: function(){
  ...
  ds.GraphTraversal.DLBF = ds.GraphTraversal.DepthLimitedBF = ds.GraphTraversal.BreadthFirst.extend('DepthLimitedBreadthFirst',{
    init: function(processor,depth){
      this.Super(processor);
      this.depth  = this._dp = depth;
    },
  ...
  ds.GraphFilterCore = as.Class.create('GraphFilter',{
      init: function(processor){
        this.graph = null;
        this.key = null;
  ...
  ds.GraphFilter.DepthFirst = ds.GraphFilterCore.extend('DepthFirstFilter',{
  ...
  ds.GraphFilter.BreadthFirst = ds.GraphFilterCore.extend('BreadthFirstFilter',{
    init: function(fn,depth){
      if(util.isFunction(depth)){
          var fnv = fn;
          fn = depth;
